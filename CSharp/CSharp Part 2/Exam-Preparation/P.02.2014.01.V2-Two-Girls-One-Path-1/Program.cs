    ...
    class TGOP
    {
        ...
        static void StartGame()
        {
            ...
            while (true)
            {
                ...
                {
                    //path[mollyPosition] = 1; // при odd number off flowers will have always 1 left, which is odd and will remain 1 till end (possible infinity loop if all positions have odd number of flowers)
                    if (mollyPosition % 2 == 0)
                    {
                        path[mollyPosition] = 0;
                    }
                    else
